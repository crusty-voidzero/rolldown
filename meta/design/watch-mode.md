...
File change detected by per-task FsWatcher
  ...
  → run_build_sequence(changes):
      ...
      5. For each task with needs_rebuild:
         ...
         b. task.build():
            - bundler.write_for_watch(skip_write, |scan_files| update_watch_files(scan_files))
              1. scan_modules() → discover module graph
              2. on_scan_complete callback → register FS watches (before render)
              3. bundle_write() or bundle_generate() (if skip_write)
...
`Bundler::write_for_watch(skip_write, on_scan_complete)` splits the build:

1. **Scan** — `bundle.scan_modules()` discovers module graph and populates watch files
2. **Callback** — `on_scan_complete(watch_files)` is called with discovered files.
   The watcher registers FS watches here, BEFORE render hooks fire.
   Called even if scan fails — so files are watched for error recovery.
3. **Write/Generate** — `bundle_write()` or `bundle_generate()` (if `skip_write`)

This matches the old watcher's approach, where `watch_files()` was called between scan and write phases.
...
`BindingWatcher` uses a `Mutex<BindingWatcherState>` state machine: `Pending → Running → Closed`. The `closed_notify` `Arc<Notify>` is stored separately so `wait_for_close()` can await it without holding the state mutex across await points.

### Event Emitter

`WatcherEmitter` uses a simple `Map<string, Function[]>` for listener storage (on/off). Async `emit()` dispatches via `Promise.all()`, matching Rollup's concurrent handler execution semantics. No external dependency needed.

### Event Mapping

Lives in `watcher.ts` (`createEventCallback()`), not in the emitter. Maps `BindingWatcherEvent` → Rollup-compatible event objects. Error events use `new Error(errors.join('\n'))` since the new watcher's error data arrives as pre-formatted `Vec<String>` rather than `Vec<BuildDiagnostic>`.
...
WatchCoordinator.run_build_sequence()
  ...
  → JS: createEventCallback() receives BindingWatcherEvent
    ...
    → emitter.emit('event', mapped_event) → Promise.all(handlers)
...
- [x] Async `emit()` via `Promise.all()` matching Rollup's concurrent handler semantics
...
- [x] `skipWrite` support — `write_for_watch(skip_write, ...)` calls `bundle_generate()` instead of `bundle_write()` when `skip_write` is true
